...
const count = (target, input) => {
  return input.reduce((sum, arr) => sum += arr.reduce((arrSum, value) => target === value ? arrSum + 1 : arrSum, 0), 0);
};
...
const totalSum = (input) => {
  return input.reduce((sum, arr) => sum + arr.reduce((arrSum, value) => arrSum + value, 0), 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(arr => {
    arr = arr.filter(value => typeof value === 'number' && value % 5 === 0);
...
let findMaleAndFemale = (data) => {
  let names = data.filter(person => person.gender === 'male' || person.gender === 'female').map(person => person.name);
...
let findShortest = (data) => {
  return data.reduce((shortest, person) => person.name.length <= shortest.length ? person.name : shortest, data[0].name);
};
